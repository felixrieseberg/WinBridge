    ...
    internal class MobileServiceRequestHelper<T> where T : class
    {
        ...
        public MobileServiceRequestHelper(string azureEndPoint, string token, MobileServiceUser User)
        {
           ...
           // ServicePointManager.ServerCertificateValidationCallback = (p1, p2, p3, p4) => true;
            ...
            _client.BaseUrl = _azureEndPoint;
            _client.AddHeader("Accept", "application/json");
            _client.AddHeader("X-ZUMO-APPLICATION", token);
             
            _user = User;
            if (_user != null)
            {
                _client.AddHeader("X-ZUMO-AUTH", _user.AuthenticationToken);
            }
        }
...
/*
        ...
        private void LoginAsyncHandler(RestResponse<MobileServiceUser> restResponse, RestRequestAsyncHandle handle)
        {
...
*/
...
#endif
        /*
        private void CheckError(RestResponse restResponse)
        {
        ...
        public void GetAsyncHandler(RestResponse<List<T>> restResponse, RestRequestAsyncHandle handle)
        {
...
//                || restResponse.StatusCode == HttpStatusCode.Created)
        ...
        private static AzureResponse<List<T>> DeserialiseObjectList(IRestResponse restResponse)
        {
            ...
            {
                response = new AzureResponse<List<T>>(restResponse.Request.ErrorException);
            }
            return response;
        }
*/
        public async void QueryAsync(Expression<Func<T, bool>> predicate, Action<AzureResponse<List<T>>> callback)
        {
            ODataExpressionConverter c = new ODataExpressionConverter();
            var query = c.Convert<T>(predicate);

            var request = new RestRequest(_tableName + "?$filter=" + query, System.Net.Http.HttpMethod.Get);
//            var request = new RestRequest("LeaderBoard" + "?$filter=UserName ne null", System.Net.Http.HttpMethod.Get);

            var data = await _client.ExecuteAsync<List<T>>(request);
            var res = new AzureResponse<List<T>>(AzureResponseStatus.Success, data);
            callback(res);
//            var response = _client.ExecuteAsync<List<T>>(request).Result;//, (IRestResponse<List<T>> response) =>
            //{
